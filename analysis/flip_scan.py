from analysis.enchant_profit import evaluate_flip, get_item_enchant_level
from scanner.scan import generate_enchants

def run_flip_scan(base_items, lowest_prices):
    results = []

    for base in base_items:
        enchants = generate_enchants(base)

        for target in enchants:
            if target == base:
                continue
            result = evaluate_flip(base, target, lowest_prices)
            if result["ok"]:
                results.append(result)

        if enchants[1] in lowest_prices:
            result = evaluate_flip(enchants[1], enchants[2], lowest_prices)
            if result["ok"]:
                results.append(result)

            result = evaluate_flip(enchants[1], enchants[3], lowest_prices)
            if result["ok"]:
                results.append(result)

        if enchants[2] in lowest_prices:
            result = evaluate_flip(enchants[2], enchants[3], lowest_prices)
            if result["ok"]:
                results.append(result)
    results.sort(key=lambda r: r["profit"], reverse=True)
    return results